...
def write_to_excel(template_path, output_path, df):
    ...
    import re
    month_match = re.search(r'_(\d{4})_(\d{2})', output_path)
    month_value = int(month_match.group(2)) if month_match else 2  # CSVファイル名から月を取得
    year_value = int(month_match.group(1)) if month_match else sheet["F5"].value
    sheet["H5"] = month_value
    
    for index, row in enumerate(df.itertuples(), start=11):  # C列から開始
        day_value = index - 10  # A11に1日から入力
        sheet[f"A{index}"] = f"=DATE({year_value},{month_value},{day_value})";  # C列から開始
        sheet[f"A{index}"] = f"=DATE({year_value},{month_value},{index - 8})"
