...
def write_to_excel(template_path, output_path, df, csv_filename):
    ...
    # CSVファイル名から従業員名を抽出し G1 セルに記載
    import re
    name_match = re.search(r'勤怠詳細_(.+?)_\d{4}_\d{2}', csv_filename)
    ...
    for index, row in enumerate(df.itertuples(), start=11):  # C列から開始
        ...
        sheet[f"E{index}"] = "1:00" if row.勤怠種別 not in ["未入力", "所定休日", "法定休日"] else ""  # 休憩時間固定（E列に記述）
