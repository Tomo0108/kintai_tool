...
csv_files = [f for f in os.listdir(data_dir) if f.endswith(".csv")] # type: ignore
if not csv_files:
    raise FileNotFoundError("データフォルダにCSVファイルが見つかりません。")
latest_csv = sorted(csv_files)[-1]  # 一番新しいファイルを選択
csv_path = os.path.join(data_dir, latest_csv) # type: ignore
...
output_path = os.path.join(data_dir, output_filename) # type: ignore
