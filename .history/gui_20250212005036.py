...
import subprocess
from config import TEMPLATE_PATH, CONFIG_FILE
import configparser

def select_csv():
    """CSVファイルを選択する"""
    file_path = filedialog.askopenfilename(initialdir=INPUT_DIR, title="CSVファイルを選択", filetypes=[("CSV files", "*.csv")]) # type: ignore
    if file_path:
