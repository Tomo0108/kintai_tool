...
# 設定を読み込む
config = load_config()
default_name = config['Settings'].get('DEFAULT_EMPLOYEE_NAME', "")
default_template = config['Settings'].get('TEMPLATE_PATH', TEMPLATE_PATH)

# GUIウィンドウの作成
root = tk.Tk()
root.title("勤怠表作成ツール")
root.geometry("420x00")
...
template_entry.insert(0, default_template)  # 設定ファイルから取得
template_entry.pack()
...
name_entry.insert(0, default_name)  # 設定ファイルから取得
name_entry.pack()
